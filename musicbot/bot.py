...
class MusicBot(discord.Client):
    ...
    async def update_now_playing_status(self, entry=None, is_paused=False):
        ...
        game = """
*help
Thanks for using BetaBot
If you need help use the *help command
If you need help contact the devs
Made By Fsxstreamer123"""
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            helpmsg += "```"
            helpmsg += " created by fsxstreamer123"
    ...
    async def cmd_clearbug(self, author, server, channel):
        if server.id == "273126432337821696":  # my server
            print("Server is correct")
            perms = author.permissions_in(channel)
            for role in author.roles:
                try:
                    if perms.administrator or perms.manage_server or perms.manage_channels:  # Devs and helpers
                        usage = True
                    else:
            ...
            if author.id == "220568440161697792" or "223814431946178560":
                pass
            else:
                return Response("You cant use this command")
    ...
    async def cmd_bug(self, channel, server, author, message):
        ...
        if server.id not in bugger:
            ...
            try:
                if shard_id == 0:
                    await self.safe_send_message((discord.Object(id='273127326202724354')), (msg))
                if shard_id == 1:
                    await self.safe_send_message((discord.Object(id='273127326202724354')), (msg))
            except:
                return Response(
                    "Something very bad has happened which technically shouldnt be able to happen. Type /join and join my server, mention helpers or devs and say you hit **ERROR 666**")
            text = " " + server.id
            bugged.write(text)
            print(bugged)
            bugged.close()
    ...
    async def cmd_kidnap(self, message):
        return Response("Im_hardertokidnap's twitch link is: https://www.twitch.tv/im_hardertokidnap come check her out")

    async def cmd_catvid(self, message):
        return Response("https://procatinator.com/")
    ...
    async def cmd_ststushelp():
        await self.update_now_playing_status("*help")
    ...
    async def cmd_sendall(self, args, leftover_args, channel, message): 
         """ 
         Usage: 
         ...
         return Response("Your messsage has been sent to all servers")
    ...
    async def cmd_start(self, author, channel):
        for s in self.servers:
            await self.safe_send_message(s, "The bot has been started (by {0.name}#{0.discriminator})".format(author))

    async def cmd_shutdown(self, author, channel):
        for s in self.servers:
            await self.safe_send_message(s, "Shutting down (by {0.name}#{0.discriminator})".format(author))
        await self.disconnect_all_voice_clients()
        raise exceptions.TerminateSignal

    async def cmd_update(self, author, channel):
        for s in self.servers:
            await self.safe_send_message(s, "Updating (by {0.name}#{0.discriminator})".format(author))
        await self.disconnect_all_voice_clients()
        raise exceptions.TerminateSignal

    async def cmd_silentupdate(self, channel):
        await self.safe_send_message(channel, ":wave:")
        await self.disconnect_all_voice_clients()
        raise exceptions.TerminateSignal

    async def cmd_silentrestart(self, channel):
        await self.safe_send_message(channel, ":wave:")
        await self.disconnect_all_voice_clients()
        raise exceptions.RestartSignal

    async def cmd_silentshutdown(self, channel):
        await self.safe_send_message(channel, ":wave:")
        await self.disconnect_all_voice_clients()
        raise exceptions.TerminateSignal

    async def cmd_live(self, server):
        return Response("@everyone Im_hardertokidnap is live on twitch.tv her link is: https://www.twitch.tv/im_hardertokidnap ")
